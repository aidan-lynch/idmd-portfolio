...
function getNumDays(monthIndex) {
  //says total day vairables and increases it bases on days in a month so that we can move on to the next column after recahing that value
var dayT = 0
var count = 0
for (i = 0; i < monthIndex; i++) {
  dayT = dayT + getDaysInMonth(i);
  }
  return dayT
}
function getDaysInMonth(month) {
  //moves on to next column depending on what month value it is based on day total
  if (month == 1) {
    return 29;
